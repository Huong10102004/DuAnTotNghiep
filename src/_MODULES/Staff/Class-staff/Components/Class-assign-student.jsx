...
const ClassAssignStudent = () => {
  ...
  const [unassignedStudents, setUnassignedStudents] = useState([
    { id: 1, name: 'Nguyễn Duy Kiên', birthDate: '01/01/2024', class: 'Chưa có', grade: 'Khối 6', selected: false, key: 1 },
    { id: 2, name: 'Nguyễn Duy Kiên', birthDate: '01/01/2024', class: 'Chưa có', grade: 'Khối 6', selected: false, key: 2 },
    { id: 3, name: 'Nguyễn Duy Kiên', birthDate: '01/01/2024', class: 'Chưa có', grade: 'Khối 6', selected: false, key: 3 },
    // Thêm học sinh khác...
  ]);
  ...
  const assignStudents = () => {
    const selectedStudents = unassignedStudents.filter(student => student.selected);
  ...
  const handleCheckAll = () => {
    ...
    setUnassignedStudents(unassignedStudents.map(student => ({ ...student, selected: newCheckAll })));
  };
    ...
    <div>
           <div className="pt-6rem bg-white h-100vh px-4">
            ...
            <div className="d-flex align-items-end mt-2">
              <p>Bạn đã chọn<span className="fw-700"> 03 học sinh </span>. Bạn vui lòng chọn lớp dự định chuyển tới và click vào nút: gán học sinh để hoàn tất công việc</p>
                <button className="btn btn-primary py-1 d-flex align-items-center ms-3" onClick={assignStudents}><img src={icon_assign_student} className="me-2" /> Gán học sinh vào lớp học</button>
            </div>

            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '20px 0' }} className="mt-10">
                {/* Bảng học sinh chưa gán */}
                <div style={{ width: '45%'}} className="border px-2">
                    <h3 className="mb-4 mt-3 fw-700">Học sinh chưa gán vào lớp</h3>
                    ...
                    <div className="d-flex my-3 justify-content-between align-items-center">
                        ...
                        <input placeholder={t('search')} className={`px-3 py-2 border-radius-3px w-300px border`}/>
                    </div>
                    {/* <Table dataSource={unassignedStudents} columns={columnsUnassigned} pagination={false} rowKey="id" /> */}
                    <Table className={styles.customTable} columns={columnsUnassigned} dataSource={unassignedStudents} pagination={{pageSize: 50,}}scroll={{y: 55 * 5,}}/>
                </div>
                ...
                <div style={{ width: '45%' }} className="border px-2">
                    <h3 className="mb-4 mt-3 fw-700">Học sinh lớp 6A1</h3>
                    <div style={{ marginBottom: '20px' }}>
                    <div className="row">
                            <div className="col-sm-1">
                                <span>Khối</span>
                    ...
                    <Table dataSource={assignedStudents} columns={columnsAssigned} pagination={false} rowKey="id" />
                </div>
            </div>
      </div>
    </div>
